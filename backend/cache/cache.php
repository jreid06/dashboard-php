...
class Cache
{
    private $_salt = 'jason'; // <-- change before usage!
    ...
    {
        if (md5($this->_salt) == "233abed7ee9945c0429047405d864283") {
            throw new Exception("Change _salt value before usage! (line 5)");
        }

        if ($name == null) {
            throw new Exception("Invalid name argument (empty or null)");
        }
        if ($dir == null) {
            throw new Exception("Invalid dir argument (empty or null)");
        }
        if ($extension == null) {
            throw new Exception("Invalid extension argument (empty or null)");
        }
    ...
    {
        if (!is_array($json)) {
            throw new Exception("Invalid cache (not an array?)");
        }
    ...
    {
        return ROOT_PATH.'cache/'.$this->_dir . md5($this->_name . $this->_salt) . $this->_extension;
    }
